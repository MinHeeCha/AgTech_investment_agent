...
AgTech 스타트업의 투자 가능성을 평가하는 종합 멀티에이전트 AI 시스템입니다. 8개의 전문 에이전트를 **LangGraph StateGraph** 기반 5단계 파이프라인으로 조율하여 기술력, 시장성, 임팩트, 경쟁 포지션 등 투자 의사결정에 필요한 핵심 요소를 분석합니다.
...
본 시스템은 **8개의 전문 에이전트**로 구성된 **LangGraph StateGraph 기반 5단계 파이프라인**을 따릅니다:

```
Stage 0: 스타트업 발굴 (node_discovery)
    ↓
Stage 1: 병렬 분석 fan-out (LangGraph 병렬 노드)
    ├─ node_tech   — 기술 요약
    ├─ node_market — 시장성 평가
    ├─ node_impact — 임팩트 평가
    └─ node_moat   — 데이터 해자 분석
    ↓ (fan-in)
Stage 2: top-3 선정 + 경쟁사 비교 (node_competitor)
    ↓
Stage 3: 투자 결정 (node_decision) — top-3 기업만 대상
    ↓ (조건 분기)
Stage 4: 보고서 생성
    ├─ node_report_single  — 임계값 통과 기업 ≥1개: 최고점 1개 보고서
    └─ node_report_all     — 임계값 통과 기업 없음: top-3 전체 투자 비권고 보고서
```
...
#### **7. InvestmentDecisionAgent** (Stage 3)
- top-3 기업에 한해 실행
- 이전 모든 에이전트의 증거 집계
- 종합 평가 점수 산출
- 신뢰도 포함 투자 권고안 도출
- **출력**: `InvestmentDecision` (투자 권고 / 추가 검토 필요 / 투자 비권고)

#### **8. ReportGenerationAgent** (Stage 4)
- 임계값(0.60) 통과 기업이 있으면 최고점 1개 보고서 생성
- 통과 기업이 없으면 top-3 전체에 대해 투자 비권고 보고서 3개 생성
- 모든 에이전트 분석 결과 요약, 증거 및 결정 근거 문서화
- **출력**: `FullEvaluationResult` + PDF 형식화 보고서
...
| 카테고리 | 가중치 | 세부 기준 |
|----------|--------|-----------|
| **기술력** | 20% | 신규성, 방어력, 실현 가능성 |
| **시장성** | 25% | 시장 규모, 성장 잠재력, 상업적 실현 가능성 |
| **임팩트** | 15% | 환경적 혜택, 농업적 혜택, 측정 가능성 |
| **데이터 해자** | 20% | 데이터 자산, 네트워크 효과, 방어력 |
| **경쟁 포지션** | 20% | 차별화, 진입 장벽, 시장 포지션 |

### 투자 결정 임계값

| 점수 | 권고 | 결정 |
|------|------|------|
| ≥ 0.75 | **투자 권고 (강력)** | 강한 신호를 가진 명확한 투자 기회 |
| 0.60 - 0.74 | **투자 권고 (조건부)** | 충분한 증거를 가진 합리적인 기회 |
| 0.40 - 0.59 | **추가 검토 필요** | 유망하나 추가 평가 필요 |
| < 0.40 | **투자 비권고** | 현 시점 투자 비권고 |
...
│   ├── __init__.py
│   ├── config.py              # 설정 관리
│   ├── graph.py               # LangGraph StateGraph 정의 (노드 · 엣지 · 조건 분기)
│   ├── orchestrator.py        # LangGraph 실행 래퍼 (AgentOrchestrator)
...
```bash
# LLM 설정 (에이전트 분석 · LLM polish에 OpenAI gpt-4o-mini 사용)
...
✅ **병렬 실행** — LangGraph fan-out으로 Stage 1에서 4개 에이전트 병렬 실행
...
### 병렬 실행
Stage 1 분석 에이전트(tech, market, impact, moat)는 **LangGraph fan-out** 방식으로 병렬 실행됩니다. 각 노드가 독립적으로 `node_discovery` 완료 후 동시에 시작되며, 모두 완료되면 `node_competitor`로 fan-in됩니다.
